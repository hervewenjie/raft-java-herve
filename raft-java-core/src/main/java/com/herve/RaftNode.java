...
import com.herve.storage.SegmentedLog;
import com.herve.storage.Snapshot;
...
/**
 * Main class of raft, provides
 * 1. store info (status, log, snapshot)
 * 2. send rpc request
 * 3. timer
 *
...
public class RaftNode {
    ...
    @Getter @Setter private RaftOptions raftOptions;
    @Getter @Setter private RaftMessage.Configuration configuration;
    @Getter @Setter private ConcurrentMap<Integer, Peer> peerMap = new ConcurrentHashMap<>();
    @Getter @Setter private RaftMessage.Server localServer;
    @Getter @Setter private StateMachine stateMachine;
    @Getter @Setter private SegmentedLog raftLog;
    @Getter @Setter private Snapshot snapshot;

    // start as follower
    private NodeState state = NodeState.STATE_FOLLOWER;
    // last known term, init 0
    ...
    public void init() {
        ...
        // start election(pre vote) after random timeout
        resetElectionTimer();
    ...
    private void resetElectionTimer() {
        // if last task not completed, cancel it
    ...
    /**
     * Client init one pre-vote request
     * pre-vote/vote is typical 2pc
     * In case one node's networking fails, it keeps increasing term number,
     * when it's back, causing other nodes in cluster term number change
     *
     */
    ...
    private class PreVoteResponseCallback implements RPCCallback<RaftMessage.VoteResponse> {
        ...
        public void success(RaftMessage.VoteResponse response) {
            lock.lock();
            try {
                peer.setIsVoteGranted(response.getGranted());
                ...
                if (response.getTerm() > currentTerm) {
                    ...
                    if (response.getGranted()) {
                        ...
                        for (RaftMessage.Server server : configuration.getServersList()) {
                            ...
                            if (peer1.getIsVoteGranted() != null && peer1.getIsVoteGranted() == true) {
                                voteGrantedNum += 1;
                            }
                        }
        ...
        public void fail(Throwable e) {
            LOG.warn("pre vote with peer[{}:{}] failed",
                    peer.getServer().getEndPoint().getHost(),
                    peer.getServer().getEndPoint().getPort());
            peer.setIsVoteGranted(new Boolean(false));
        }
    }
